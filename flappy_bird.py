...
def draw_window(win, birds, pipes, base, score, gen):
    ...
    win.blit(text1, (WIN_WIDTH - 300 - text.get_width(), 10))
...
def main(genomes, config):
    ...
    while run:
        ...
        for curr_pipe in pipes:
            for x, bird in enumerate(birds):
                if curr_pipe.collide(bird):
                    ge[x].fitness -= 1
                    birds.pop(x)
                    nets.pop(x)
                    ge.pop(x)

                if not curr_pipe.passed and curr_pipe.x < bird.x:
                    curr_pipe.passed = True
                    add_pipe = True

            if curr_pipe.x + curr_pipe.PIPE_TOP.get_width() < 0:
                rem.append(curr_pipe)
